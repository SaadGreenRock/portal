...
import type { RfqQuery } from "@/lib/rfq/types";
...
const STATUSES = new Set<string>(["draft", "sent", "closed", "cancelled", "open", "deleted"]);
...
export default async function RfqHistory({
  ...
  const status = (STATUSES.has(sp.status ?? "") ? sp.status : "all") as RfqQuery["status"];
  ...
  const listed = await tryTable(() => db.searchRfqs(query));
  if (!listed.ok) return <ModuleUnavailable module="Quotations" />;
  const { rows, total } = listed.value;
  const pages = Math.max(1, Math.ceil(total / PAGE_SIZE));
  const filtered = Boolean(sp.q || sp.from || sp.to || status !== "all");
  ...
  return (
    <>
      {sp.deleted ? (
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-ink-line bg-white p-4">
          <p className="text-[13.5px]">
            ...
            spent and will not be reissued.
          </p>
          <Link
            href={`/${company.slug}/rfq/history?status=deleted`}
      ...
      <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-[20px] font-bold tracking-tight">Quotation request history</h1>
          <p className="mt-1 text-[14px] text-ink-soft">
            Every request ever raised for {company.name}.
          </p>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <div className="sm:col-span-2">
            ...
            <input
              ...
              name="q"
              defaultValue={sp.q ?? ""}
              placeholder="Request no., subject, note…"
              className="input"
            />
          </div>
      ...
      {rows.length === 0 ? (
        <div className="card px-6 py-14 text-center">
          <p className="text-[15px] font-medium">
            {filtered ? "No requests match those filters." : "No requests yet."}
          </p>
          <p className="mt-1.5 text-[13.5px] text-ink-soft">
            {filtered
              ? "Try widening the date range."
              : `The first ${company.name} request will appear here once raised.`}
          </p>
        </div>
      ) : (
        <>
